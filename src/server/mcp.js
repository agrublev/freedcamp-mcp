...
const tools = [
    { name: "fc_fetch_task", description: "Get a single Freedcamp task by ID", schema: tasks.FetchTaskSchema, outputSchema: TaskSchema },
    { name: "fc_fetch_tasks", description: "List Freedcamp tasks with optional filters", schema: tasks.FetchTasksSchema, outputSchema: TaskListResponseSchema },
    ...
    { name: "fc_fetch_file", description: "Get file metadata by ID", schema: files.FetchFileSchema },
    { name: "fc_add_file_meta", description: "Create a file metadata record for a file already uploaded elsewhere", schema: files.AddFileMetaSchema },
    { name: "fc_upload_file", description: "Upload a file and attach it to a project or item", schema: files.UploadFileSchema },
    { name: "fc_delete_file", description: "Delete a file", schema: files.DeleteFileSchema },
    { name: "fc_upload_avatar", description: "Upload an avatar image for the current user", schema: files.UploadAvatarSchema },
...
export function createMcpServer(fc) {
    const server = new Server({ name: "freedcamp-mcp-server", version: VERSION }, { capabilities: { tools: {} } });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            switch (name) {
                ...
                case "fc_fetch_file": return ok(await fc.fetchFile(files.FetchFileSchema.parse(args)));
                case "fc_add_file_meta": return ok(await fc.addFileMeta(files.AddFileMetaSchema.parse(args)));
                case "fc_upload_file": return ok(await fc.uploadFile(files.UploadFileSchema.parse(args)));
                case "fc_delete_file": return ok(await fc.deleteFile(files.DeleteFileSchema.parse(args)));
                case "fc_upload_avatar": return ok(await fc.uploadAvatar(files.UploadAvatarSchema.parse(args)));
